...
np.savetxt('20newsgroup_Train_Labels.txt',newsgroups_train.target) 
...
embedding_vector = np.empty([len(newsgroups_train.data),model.embeddings.word_embeddings.embedding_dim])

for i in range(len(newsgroups_train.data)) :
    
    #Split each sentence and generate word embedding and mean for all sentences in file 
    data_no_newline = newsgroups_train.data[i].replace('\n','') #Replace new line characters 
...
np.savetxt('20newsgroup_BERT_Embedding.txt',embedding_vector)        
